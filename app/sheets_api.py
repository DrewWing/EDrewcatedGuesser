...
from common_resources import PATH_TO_FTCAPI, SEASON_YEAR, SERVICE_ACCOUNT_FILE, SPREADSHEET_ID, create_logger
...
def push_rankings(service):
    ...
    if DEBUG_LEVEL>1:
        logger.debug("  This function uses:")
        logger.debug("    -",os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"eventdata","event_rankings.json"))
        logger.debug("  This function creates:")
        logger.debug("    -",os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"eventdata","event_rankings.csv"))
