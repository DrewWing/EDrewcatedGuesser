...
# Internal Imports
import json_parse
from common_resources import PATH_TO_FTCAPI, NUMBER_OF_DAYS_FOR_RECENT_OPR, EVENT_CODE, PATH_TO_JOBLIB_CACHE, DO_JOBLIB_MEMORY, SEASON_YEAR
...
def loadTeamNumbers() -> list:
    ...
    """
    return list(pd.read_csv(os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"team_list_filtered.csv"))["teamNumber"])
...
def loadMatches(filter_by_teams: list = []):
    ...
    \nRelies on generatedfiles/{SEASON_YEAR}/all_matches.csv
    \nParameters:
    ...
    all_matches = pd.read_csv(os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"all_matches.csv")) # Get the csv data
...
def build_m(load_m: bool, matches: pd.DataFrame, teams: list) -> numpy.matrix:
    ...
    if load_m:
        ...
        M = numpy.load(os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"OPR-m.npy"))
        ...
        numpy.save(os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"OPR-m"),M)
    ...
    if (logger.isEnabledFor(logging.DEBUG)):
        ...
        #TODO re-enable this?
        # if (DEBUG_LEVEL>3):
        #     logger.debug(f"  Saving M to generatedfiles/{SEASON_YEAR}M_debug.csv for debug purposes... (this could take a little bit if it is big)")
        #     numpy.savetxt(os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"M_debug.csv"), M, delimiter=",")
        #     logger.debug("  Saved.")
...
def master_function(memory=memory):
    ...
    if do_opr_global:
        ...
        do_all_opr_stuff(
            matches=matches,
            teams=teams, 
            output_file_path=os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"opr","opr_global_result_sorted.csv"),  
            load_m=False
        )
    ...
    if do_opr_for_all_time:
        ...
        do_all_opr_stuff(
            matches=matches,
            teams=teams,
            output_file_path=os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"opr","opr_result_sorted.csv"),  
            load_m=False
        )
    ...
    if do_opr_recent:
        ...
        do_all_opr_stuff(
            matches=matches,
            teams=teams,
            output_file_path=os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"opr","opr_recent_result_sorted.csv"), 
            load_m=False,
            fallback="zeroes"
        )
    ...
    if do_opr_event_only:
        ...
        do_all_opr_stuff(
            matches = matches,
            teams=teams,
            output_file_path = os.path.join(PATH_TO_FTCAPI,"generatedfiles",str(SEASON_YEAR),"opr","opr_event_result_sorted.csv"),  
            load_m  = False
        )
