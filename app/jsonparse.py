...
def get_team_stats(team_number):
    ...
    all_oprs = pd.read_csv(PATH_TO_FTCAPI+'opr/opr-result-sorted.csv', index_col=False)
    ...
    all_oprs_recent = pd.read_csv(PATH_TO_FTCAPI+'opr/opr-recent-result-sorted.csv', index_col=False)
    #region debugstuff
    #print(' the loc thing:')
...
def write_needed_events(season_events, texasonly=False):
    ...
    rawevents = open(PATH_TO_FTCAPI+'opr/needed-events-raw.json','w+')
    rawevents.truncate()
    rawevents.write('{"matches":[\n')
    
    with open(PATH_TO_FTCAPI+'opr/needed-event-ids.txt','w+') as thefile:
        thefile.truncate()
...
def write_needed_teams(use_opr=False):
    ...
    eventcounter = 0
    if (not use_opr):
        thepath = PATH_TO_FTCAPI+'all-events'
    else:
        thepath = PATH_TO_FTCAPI+'opr/all-events'
    ...
    with open((PATH_TO_FTCAPI+'opr/all-teamids-involved.txt' if use_opr else 'team-ids-to-get.txt'),'w+') as thefile:
        thefile.truncate()
...
def prepare_opr_calculation(
    ...
    if specific_event==None:
        ...
        path_list = [PATH_TO_FTCAPI+'eventdata/eventmatches.json']
        l = 1
    
    else:
        if settings.debug_level>0:
            print(info_i() + '    [prepare_opr_calculation] Iterating through only one event with code '+str(specific_event))
        path_list = [PATH_TO_FTCAPI+'opr/all-events/'+str(specific_event).upper()+'.json']
        l = 1
    ...
    if settings.debug_level>1:
        print(green_check()+'    Teams assembled. Writing all team numbers to file team-list-filtered.csv...       ')
    
    matches_per_team['teamNumber'].to_csv(PATH_TO_FTCAPI+'team-list-filtered.csv', index=False)
    ...
    if settings.debug_level>0:
        print(green_check()+'    Matches assembled. Writing all teams to file all-matches.csv...       ')
    
    all_matches_pd.to_csv(PATH_TO_FTCAPI+'all-matches.csv', index=False)
...
if __name__ == "__main__" and 'get-events-global' in sys.argv:
    ...
    print(info_i()+'      Getting the season\'s data from seasondata at '+str(PATH_TO_FTCAPI+'seasondata.json'))

    write_needed_events(SeasonEvents(get_json(PATH_TO_FTCAPI+'seasondata.json')), texasonly=False)
